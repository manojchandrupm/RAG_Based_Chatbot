...
## Overview
A Retrieval-Augmented Generation chatbot that answers user queries based on Samsung appliance manuals. Uses LangChain, OpenAI LLM/embeddings, and FAISS vector database for scalable semantic search and context-aware responses and Scaled up with Qdrant Db and Indent Subagent and chathistory memory.
...
- Updated with Qdrant db
- added the indent sub Agent
- added the chathistory memory
...
- `RAG_chatbot_Using_react_agent` — vector_db_chatbot script
- `RAG_chatbot_React_sub_Agent` — Qdrant_DB_chatbot Script
- `agent_tools/` — Custom retriever tools (fridge, TV, washing machine) and more 
- `data/` — PDF manuals
